...
const animalsSortedByWeightReversed = animals.sort((animalA, animalB) => {
  if (animalA.weight < animalB.weight) {
    return 1;
  } else {
    return -1;
...
consosole.log(animalWithWeightMoreThanFivehundredExists);
...
const weightOfAllAnimalsInAfrica = animals
  ...
  .reduce((tempResult, currentAnimal) => {
    return tempResult + currentAnimal.weight;
  }, 0);

const animalinAfrica = animals.filter((animal) => {
  return animal.continents.includes("Africa");
});

// Hint: As above, but divided by the number of animals in Africa.
const averageWeightOfAllAnimalsInAfrica =
  weightOfAllAnimalsInAfrica / animalinAfrica;
